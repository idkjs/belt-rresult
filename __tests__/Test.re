...
let test_reword_error = () => {
  ...
  let reword = (`An_error) => `Another_one;
...
let test_bind = () => {
  log("Test R.bind");
  assert(R.bind(Ok(3), v => Error(string_of_int(v))) == Error("3"));
  assert(R.bind(Ok(3), v => Ok(string_of_int(v))) == Ok("3"));
  assert(R.bind(Error(1), v => Ok(string_of_int(v))) == Error(1));
  ();
};

let test_map = () => {
  log("Test R.map");
  assert(R.map(v => string_of_int(v), Ok(2)) == Ok("2"));
  assert(R.map(v => string_of_int(v), Error(2)) == Error(2));
  ();
};
...
let test_msgs = () => {
  ...
  assert(
    R.reword_error_msg(~replace=true, reword, Error(`Msg("ha")))
    == Error(`Msg("ha++")),
  );
  assert(
    R.reword_error_msg(~replace=false, reword, Error(`Msg("ha")))
    == Error(`Msg("ha\nha++")),
  );
  let pp_error = ppf =>
    fun
    | `E => Format.fprintf(ppf, "E");
...
let test_exn_trap = () => {
  ...
  let no_raise = x => string_of_int(x);
  // let do_raise = x => raise(Exit);
  let do_raise = _ => raise(Exit);
  assert(R.trap_exn(no_raise, 3) == Ok("3"));
  switch (R.trap_exn(do_raise, 3)) {
  | Ok(_) => assert(false)
...
let test_is = () => {
  ...
  assert(R.is_ok(Ok(2)));
  assert((!) @@ R.is_ok(Error(2)));
  assert(R.is_error(Error(2)));
  assert((!) @@ R.is_error(Ok(2)));
  ();
};
...
let test_converting = () => {
  ...
  assert(R.of_option(~none=() => Error("none"), Some(3)) == Ok(3));
  assert(R.of_option(~none=() => Error("none"), None) == Error("none"));
...
let test_ignoring = () => {
  ...
  assert(R.ignore_error(~use=_ => 3, Ok(4)) == 4);
  assert(R.ignore_error(~use=_ => 3, Error(4)) == 3);
  assert(R.kignore_error(~use=_ => Ok(3), Ok(4)) == Ok(4));
  assert(R.kignore_error(~use=_ => Ok(3), Error(4)) == Ok(3));
  assert(R.kignore_error(~use=_ => Error(3), Ok(4)) == Ok(4));
  assert(R.kignore_error(~use=_ => Error(3), Error(4)) == Error(3));
  ();
};
